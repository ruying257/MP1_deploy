...
class AdroitDataset(BaseDataset):
    ...
    def _sample_to_data(self, sample):
        ...
        # image = sample['img'][:,].astype(np.float32)            # 先看看原始 shape
        # # 如果你的 img 是 (T, H, W, C)，需要转成 (T, C, H, W)
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        if idx == 0 and 'image' in data['obs']:
            print('image shape before torch:', data['obs']['image'].shape)
