...
class ConditionalUnet1D(nn.Module):
    ...
    def forward(self, 
        ...
        # 部署导出时 timestep 由 wrapper 从 sample 派生，已经和 sample 同设备。
        # 避免在 trace 中记录 sample.device，否则 CPU 导出的 TorchScript 会写死 CPU。
        ...
        # broadcast to batch dimension in a way that's compatible with ONNX/Core ML
        # 同 timestep，r 在部署 wrapper 中已经跟随 sample 设备。
        rs = rs.expand(sample.shape[0]).to(dtype=sample.dtype)
